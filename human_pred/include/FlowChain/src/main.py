...
def parse_args() -> argparse.Namespace:
    ...
    ## added: for testing own dataset
    parser.add_argument("--custom_dataset_path", type=str, help="path to custom test dataset")

    return parser.parse_args()

## added: for testing own dataset
def test_custom_dataset(cfg: CfgNode, custom_dataset_path: str, visualize: bool) -> None:
    cfg.DATA.PATH = custom_dataset_path
    cfg.DATA.DATASET_NAME = 'custom'  # You can set this to any name appropriate for your custom dataset
    ...
    import json
    with open(os.path.join(cfg.OUTPUT_DIR, "metrics_custom.json"), "w") as fp:
...
def evaluate_model(cfg: CfgNode, model: torch.nn.Module, data_loader: torch.utils.data.DataLoader, visualize=False):
    ...
    if visualize:
        with torch.no_grad():
            ...
            # total is the number of trjrctories to be visualized
            for i, data_dict in enumerate(tqdm(data_loader_one_each, leave=False, total=10)):
                ...
                if visualize:
                    ...
                    ## Added: additional plot for directions and probabilities
                    visualizer.additional_plot(dict_list)
                # i is the number of trajectories to be visualized-1
                if i == 9:
                    break
...
def main() -> None:
    ...
    if args.mode == "train":
        ...
        if args.custom_dataset_path:
            test_custom_dataset(cfg, args.custom_dataset_path, args.visualize)
        else:
