        ...
        for subdir, dirs, files in os.walk(os.path.join(raw_path, desired_source, data_class)):
            for file in files:
                if file.endswith('.txt'):
                    ...
                    # A new Scene object is created with the total number of timesteps, time difference dt, a name derived from the filename, 
                    # # and an augmentation function if it's training data.
                    scene = Scene(timesteps=max_timesteps+1, dt=dt, name=file.rstrip('.txt'), aug_func=augment if data_class == 'train' else None)
                   
                    # Iterate over each unique node ID (agent ID) in the data.
                    for node_id in pd.unique(data['node_id']):

                        node_df = data[data['node_id'] == node_id]

                        node_values = node_df[['pos_x', 'pos_y']].values
                        #Skip nodes with fewer than 2 data points.
                        if node_values.shape[0] < 2:
                            continue
                        
                        # Calculate velocity (vx, vy) and acceleration (ax, ay) from positions.
                        ...
                        # Create a dictionary to store position, velocity, and acceleration data.
                        data_dict = {('position', 'x'): x,
                                     ('position', 'y'): y,
                                     ('velocity', 'x'): vx,
                                     ('velocity', 'y'): vy,
