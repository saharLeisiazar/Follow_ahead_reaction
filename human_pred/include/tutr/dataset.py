...
class TrajectoryDataset(data.Dataset):
    ...
    def __init__(self, dataset_path, dataset_name, dataset_type, translation=False, rotation=False, scaling=False, obs_len=8,
        ...
        # if dataset_type == 'txt':
        #     self.scenario_list = self.load_txt_data(dataset_path, dataset_name)
        # else:
        #     with open(dataset_path + dataset_name + '_' + dataset_type + '.pkl', 'rb+') as f:
        #         self.scenario_list = pickle.load(f)

    ####### load .txt file
    # def load_txt_data(self, dataset_path, dataset_name):
    #     file_path = os.path.join(dataset_path, dataset_name + '.txt')
    #     scenario_list = []
    #     with open(file_path, 'r') as file:
    #         for line in file:
    #             parts = line.strip().split(',')
    #             timestamp, x, y, z = float(parts[0]), float(parts[1]), float(parts[2]), float(parts[3])
    #             # Assuming a simple format where each line is a point in a trajectory
    #             # Group points into trajectories; here we assume one trajectory per file
    #             if len(scenario_list) == 0 or scenario_list[-1][0][-1][0] != timestamp:
    #                 scenario_list.append(([], [], []))  # Start a new trajectory

    #             obs_traj = scenario_list[-1][0]
    #             pred_traj = scenario_list[-1][1]
    #             if len(obs_traj) < self.obs_len:
    #                 obs_traj.append([x, y])
    #             else:
    #                 pred_traj.append([x, y])
        ...
        #     scenario_list[i] = (np.array(scenario_list[i][0]), np.array(scenario_list[i][1]), np.array([]))
