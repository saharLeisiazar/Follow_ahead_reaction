...
def main():
    ...
    ###########
    parser.add_argument('--best_path', type=str, required=True, help='./checkpoint/sfu/best_sfu_2.pth')
    ...
    # test_loader = DataLoader(test_dataset, collate_fn=test_dataset.coll_fn, batch_size=1, shuffle=False)
    ...
    if os.path.isfile(args.best_path):  # <-- Added this block
        print(f"Loading best model from {args.best_path}")
        model.load_state_dict(torch.load(args.best_path))
    else:
        raise FileNotFoundError(f"Best model file not found: {args.best_path}")
    ...
    def vis_direction_probabilities(obs_traj, direction_probabilities):
        # obs_traj: [B, obs_len, 2]
        # direction_probabilities: [B, 3] where the last dimension is [left_prob, straight_prob, right_prob]

        for i in range(obs_traj.shape[0]):
            plt.clf()
            curr_obs = obs_traj[i].cpu().numpy()  # [obs_len, 2]
            left_prob, straight_prob, right_prob = direction_probabilities[i].cpu().numpy()

            obs_x = curr_obs[:, 0]
            obs_y = curr_obs[:, 1]

            plt.plot(obs_x, obs_y, marker='o', color='green', label='Observed Trajectory')
            plt.scatter(obs_x[-1], obs_y[-1], marker='*', color='blue', s=300, label='Last Observed Point')

            # Display the direction probabilities
            plt.text(obs_x[-1], obs_y[-1] + 0.1, f'Left: {left_prob:.2f}', ha='center', color='red')
            plt.text(obs_x[-1], obs_y[-1] + 0.2, f'Straight: {straight_prob:.2f}', ha='center', color='blue')
            plt.text(obs_x[-1], obs_y[-1] + 0.3, f'Right: {right_prob:.2f}', ha='center', color='green')
    ...
    def test_direction(model, test_dataloader):
        model.eval()
        for (ped, neis, mask) in test_dataloader:
            ped = ped.cuda()
            neis = neis.cuda()
            ...
            ped_obs = ped[:, :args.obs_len]
            direction_probabilities = model(ped_obs, neis, None, mask, None, test=True)

            vis_direction_probabilities(ped_obs, direction_probabilities)
            
            for i in range(len(direction_probabilities)):
                left_prob = direction_probabilities[i][0].item()
                straight_prob = direction_probabilities[i][1].item()
                right_prob = direction_probabilities[i][2].item()
                print(f"Left: {left_prob:.2f}, Straight: {straight_prob:.2f}, Right: {right_prob:.2f}")
    ...
    ade, fde, num_traj = test(model, test_loader, motion_modes)
    print(f'Testing results - ADE: {ade}, FDE: {fde}, Number of Trajectories: {num_traj}')
    # test_direction(model, test_loader)
