...
dataset_freq = 50
desired_freq = 2
freq_ratio = dataset_freq // desired_freq

tanh_power_value = 0.5

exp_name = 'desired_freq:' + str(desired_freq) + 'tanh_power_value:'+ str(tanh_power_value)+ '_seq_length:'+str(seq_length) + '_input_length:'+str(input_length) + '_batch_size:'+str(batch_size) + '_hidden_size:'+str(hidden_size) + '_num_epochs:'+str(num_epochs) + '_learning_rate:'+str(learning_rate)
...
def generate_2d_data(seq_length, input_length):
    ...
    for dataset in ["S1", "S5", "S6", "S8", "S9", "S11"]:
        ...
        for j in range(freq_ratio):
            ind = np.arange(j, data.shape[0], freq_ratio)  # reducing the sampling rate 
            d = data[ind]
